...
export function timestamp(value: string): Timestamp {
  if (!value.trim() || Number.isNaN(Date.parse(value)) || !value.includes("T")) {
    throw new Error("Timestamp must be an ISO-8601 date-time");
  }
  return value as Timestamp;
}

/** Batch 1 monetary amounts are whole non-negative IDR units; no fractional sub-unit is modeled. */
export class Money {
  ...
  constructor(amount: bigint | number) {
    if (typeof amount === "number") {
      if (!Number.isSafeInteger(amount)) {
        throw new Error("Money number input must be a safe integer");
      }
    }
